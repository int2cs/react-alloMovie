...
const SelectedFilm = ({ movieID }) => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/movie/${movieID}?api_key=2a7bc3506d9e237e752a6e713962fea0&language=fr-FR`).then((res) => setMovie(res.data));
  }, [movieID]);

  return (
    <div className="selectedFilm">
      <h2>{movie.title}</h2>
      <img src={movie.poster_path ? `https://image.tmdb.org/t/p/w500${movie.poster_path}` : "./movie_poster.jpg"} alt={`Affiche du film ${movie.title}`} />
      <span>
        ...
        {movie.release_date ? `Date de sortie : ${dateFormat(movie.release_date)}` : "Date de sortie inconnue"}
      </span>
      <span>
        ...
        Note de {movie.vote_average}/10
      </span>
      <ul>{movie.genres ? movie.genres.map((el) => <li key={el.id}>{el.name}</li>) : ""}</ul>
      <p>{movie.overview ? movie.overview : "Aucune déscription disponible actuellement"}</p>
    </div>
  );
};
